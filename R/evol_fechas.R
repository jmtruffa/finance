...
#' #' @examples
#' df <- data.frame(
#'   date = as.Date(c("2025-07-15","2025-07-31","2025-07-15","2025-07-31")),
#'   id = c("A","A","B","B"),
#'   price = c(100, 110, 200, 190)
#' )
#' evol_fechas(df, fecha1="2025-07-15", fecha2="2025-07-31", campo="price", campo_fecha="date")
#'
evol_fechas <- function(df, fecha1, fecha2, campo, campo_fecha = "date") {
  ...
  df1 <- df %>%
    dplyr::filter(!!fecha_sym == fecha1) %>%
    dplyr::select(-all_of(c(campo, campo_fecha)), .valor_fecha1 = !!campo_sym)

  # Filtrar por fecha2 y quedarse con todos los campos
  df2 <- df %>%
    dplyr::filter(!!fecha_sym == fecha2)
  ...
  df_comparado <- df2 %>%
    dplyr::left_join(df1, by = join_keys) %>%
    dplyr::mutate(variacion_pct = round((.data[[campo]] / .valor_fecha1 - 1) * 100, 2))
