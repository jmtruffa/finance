...
inflacionbe = function(fecha, ...) {
  ...
  require(finance)
  require(methodsPPI)
  tmpCalendar <- bizdays::create.calendar('tmpCalendar', functions::getFeriados(), weekdays = c('saturday','sunday'))
  ...
  # Voy a quitar los TT (duales) por que están a tasa negativa. Son los que arrancan con TT
  lecaps = lecaps %>% filter(!str_detect(ticker, "TT"))
